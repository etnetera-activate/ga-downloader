...
function parseCommandLineAndDownloadGaData(){
    try {
        ...
        //set absolute path to auth json
        if(options._all.authFile[0]==='.'){
            options._all.authFile = path.join(path.resolve("./"),options._all.authFile);
        }
        debug("Using authfile: %s", options._all.authFile)
...
function downloadGaData(options){
    ...
    process.env.GOOGLE_APPLICATION_CREDENTIALS = options.authFile
    ...
    return new Promise((resolve, reject) => {
        ...
        jwt.authorize((err, response) => {
            getAndPrintData({
                ...
                segment: options.segment?encodeURI(options.segment):undefined,
                filters: options.filters?encodeURI(options.filters):undefined
...
function getAndPrintDataRacursively(config, options, resolve, reject){
    // debug("Downloading data recursively. %o", config)
